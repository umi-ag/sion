...
import { isBoundCheckEvent, isContainsClaimEvent } from '../moveCall/sion/membership/structs';
...
export const sionTransactionResponseResolver = {
  ...
  verifyBoundCheck: (
    ...
    const events = response.events?.filter((event) => isBoundCheckEvent(event.type)) ?? [];
