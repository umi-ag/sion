...
export function verifyHashPreimageAndRangeProof(
  txb: TransactionBlock,
  args: VerifyHashPreimageAndRangeProofArgs,
) {
  return txb.moveCall({
    target: `${PUBLISHED_AT}::verifier::verify_hash_preimage_and_range_proof`,
    arguments: [
...
export function verifyGroth16Bn254(txb: TransactionBlock, args: VerifyGroth16Bn254Args) {
  return txb.moveCall({
    target: `${PUBLISHED_AT}::verifier::verify_groth16_bn254`,
    arguments: [
      pure(txb, args.vecU81, `vector<u8>`),
      pure(txb, args.vecU82, `vector<u8>`),
      pure(txb, args.vecU83, `vector<u8>`),
